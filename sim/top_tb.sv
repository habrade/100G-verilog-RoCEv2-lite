...
module top_tb;
  ...
  localparam real CLK_PERIOD = 1000.0/322.625;  // ~3.1 ns
  localparam real DRP_PERIOD = 1000.0/125.0;    // 8 ns

  reg clk = 0;
  reg drp_clk = 0;
  reg rst = 1;

  always #(CLK_PERIOD/2) clk = ~clk;   // 322.625 MHz
  always #(DRP_PERIOD/2) drp_clk = ~drp_clk; // 125 MHz

  initial begin
    #40 rst = 0;
  end

  // CMAC RX side driving the FIFO
  ...
  // FIFO output to fpga_core
  wire [DATA_WIDTH-1:0] fifo_rx_axis_tdata;
  wire [KEEP_WIDTH-1:0] fifo_rx_axis_tkeep;
  ...
  fpga_core core_inst (
      ...
      .qsfp1_rx_axis_tdata(fifo_rx_axis_tdata),
      .qsfp1_rx_axis_tkeep(fifo_rx_axis_tkeep),
      .qsfp1_rx_axis_tvalid(fifo_rx_axis_tvalid),
      .qsfp1_rx_axis_tlast(fifo_rx_axis_tlast),
      .qsfp1_rx_axis_tuser(fifo_rx_axis_tuser),
      .qsfp1_rx_enable(),
